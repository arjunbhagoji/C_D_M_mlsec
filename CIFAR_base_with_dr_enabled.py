...
def build_cnn(input_var=None):
    ...
    network = lasagne.layers.Conv1DLayer(
            network, num_filters=64, filter_size=25,
    ...
    network = lasagne.layers.Conv1DLayer(
            network, num_filters=128, filter_size=25,
            nonlinearity=lasagne.nonlinearities.rectify)
    network = lasagne.layers.Conv1DLayer(
            network, num_filters=128, filter_size=25,
            nonlinearity=lasagne.nonlinearities.rectify)
    network = lasagne.layers.MaxPool1DLayer(network, pool_size=42)
...
def main(NUM_EPOCHS=50):
    ...
    X_train_mod=X_train-np.array([X_train_mean,]*50000)
    pca=PCA(n_components=rd,whiten=True)
    X_train_dr=pca.fit_transform(X_train_mod)
    X_train_dr=X_train_dr.reshape((50000,1,rd))

    X_test=X_test.reshape((10000,3072))
    #X_test_mean=np.mean(X_test,axis=0)
    X_test_mod=X_test-np.array([X_train_mean,]*10000)
    X_test_dr=pca.transform(X_test)
    X_test_dr=X_test_dr.reshape((10000,1,rd))
    ...
    if model_exist_flag==0:
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            X_train_dr = X_train_dr[train_indices,:,:]
        ...
        # np.savez('CIFAR_model_FC10_'+str(DEPTH)+'_'+str(WIDTH)+'_drop.npz',
        #         *lasagne.layers.get_all_param_values(network))
        np.savez(abs_path_m+'cifar10_cnn_model_no_reg_no_aug_trial_'+str(rd)+'.npz',
                    *lasagne.layers.get_all_param_values(network))
    elif model_exist_flag==1:
        # load network weights from model file
        with np.load(abs_path_m+'cifar10_cnn_model_no_reg_no_aug_trial_'+str(rd)+'.npz') as f:
             param_values = [f['arr_%d' % i] for i in range(len(f.files))]
        # with np.load(abs_path_m+'CIFAR_model_FC10_'+str(DEPTH)+'_'+str(WIDTH)+'_drop.npz') as f:
        #      param_values = [f['arr_%d' % i] for i in range(len(f.files))]
    ...
    #testfile.write('Model:CIFAR_model_FC10_'+str(DEPTH)+'_'+str(WIDTH)+'_drop')
    testfile.write('Model: model_cnn_9_layers_papernot_no_reg_no_aug_flat_trial'+'\n')
    testfile.write("reduced_dim: "+str(rd)+"\n"+"Epochs: "
                +str(NUM_EPOCHS)+"\n"+"Test accuracy: "
                +str(avg_test_acc)+"\n")
    testfile.write("#####################################################"+
                    "####"+"\n")

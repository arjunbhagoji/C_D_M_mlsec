...
for item in list_dim:
    ...
    path_curr=abs_path_o+'Gradient_attack_SVM_rand_retrain'+str(item)+'.txt'
    curr_array=np.genfromtxt(path_curr,delimiter=',',skip_header=0)
    handle_list.append(plt.plot(curr_array[:,1],curr_array[:,4],linestyle='-', marker=style, color=color, markersize=10,label=int(curr_array[0,0])))
...
plt.title('Re-training defense with random projections for MNIST data \n Model: Linear SVM')
# plt.ylim(0.0,100.0)
# plt.xlim(0.01,0.1)
plt.xticks()
...
#plt.show()
plt.savefig(abs_path_p+'MNIST_SVM_rand_retrain_pure_adv.png', bbox_inches='tight')
